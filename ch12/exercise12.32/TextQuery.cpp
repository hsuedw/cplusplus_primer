...
void TextQuery::dbgPrint()
{
    lineNumType i = 0;
    cout << "text: " << endl;
    for (StrBlobPtr it = text.begin(); neq(it, text.end()); it.incr()) {
        cout << "(line " << i << ") " << it.deref() << endl;
	++i;
    }
    cout << endl;
...
QueryResult TextQuery::query(const string &word)
{
	...
	return QueryResult(word, text.begin(), noData);
    }

    // word is found.
    return QueryResult(word, text.begin(), iter->second);
}
...
ostream &print(ostream &os, const QueryResult &qr)
{
    ...
    for (auto &num : *(qr.lineNum)) {
        os << "   (line " << num << ") " << qr.text.deref(num) << endl;
    }
